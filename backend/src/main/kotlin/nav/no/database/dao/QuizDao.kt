...
class QuizDao(
    ...
    fun getQuiz(id: Long): Quiz {
        dataSource.connection.use {
            val rs = it.prepareStatement(SELECT_QUIZ).apply { setLong(1, id) }.executeQuery()
            return if (rs.next()) {
                Quiz(
                        rs.getString("name"),
                        rs.getLong("id"),
    ...
    fun createQuiz(quiz: CreateQuizRequest): Long =
            dataSource.connection.use {
                return it.prepareStatement(POST_QUIZ)
                        .apply {
    ...
    fun updateQuiz(quiz: Quiz) {
        dataSource.connection.use {
            it.prepareStatement(UPDATE_QUIZ)
                    .apply {
                        setString(1, quiz.name)
                        setString(2, quiz.description)
    ...
    fun deleteQuiz(id: Long) {
        dataSource.connection.use {
            it.prepareStatement(DELETE_QUIZ).apply { setLong(1, id) }.executeQuery()
        }
    }
